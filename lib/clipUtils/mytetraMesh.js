let Octree = require('./octree').Octree;
let Plane = require('./plane').Plane;
let Aabb = require('./aabb').Aabb;
...
function TetraMesh(gl) {
    ...
    this.initTetraMesh = function(positions, tetraIndex, data){
        this.nodeArray_  = positions;
        this.tetraArray_ = tetraIndex;
        this.dataArray_  = data;
    ...
    this.computeOctree = function() {
        ...
        for (let i = 0; i < nbTetras; ++i)
        {
            ...
            let v4x = vAr[ind4],
                v4y = vAr[ind4 + 1],
                v4z = vAr[ind4 + 2];
            Geometry.computeTetraAabb(tetrasAabb[i],
                                      v1x, v1y, v1z,
                                      v2x, v2y, v2z,
                                      v3x, v3y, v3z,
                                      v4x, v4y, v4z);
    ...
    this.makeSlice = function(a, b, c, d) {
        ...
        for(let i = 0; i < nbTetrasCandidates; ++i) {
            ...
            if ( Math.abs(dot1 + dot2 + dot3 + dot4) == 4) continue;
            v1 = interPoints[0];
            v2 = interPoints[1];
            v3 = interPoints[2];
            let s1 = interScalar[0];
            let s2 = interScalar[1];
            let s3 = interScalar[2];
        ...
        let nAr = new Float32Array(vArray.length);
        let nx = normal[0],
            ny = normal[1],
            nz = normal[2];
        for(let i = 0; i < nbPoints; ++i)
