...
/** Maximum primitives per cell */
Octree.maxIndices_ = 100;
...
Octree.prototype = {
     ...
     */
    build: function (indices, idAabb, aabb)
    {
        ...
        var cellIndices = this.indices_;
        var nbIndices = indices.length;
        for (var i = 0; i < nbIndices; ++i)
        {
            var t = idAabb[indices[i]];
            if (aabbSplit.pointInside(t.computeCenter()))
            {
                aabbLoose.expandsWithAabb(t);
                cellIndices.push(indices[i]);
            }
        }
        var nbIndicesCell = cellIndices.length;
        if (nbIndicesCell > Octree.maxIndices_ && this.depth_ < Octree.maxDepth_)
     ...
     * @param {Aabb[]} idAabb The aabb of the primitives
     */
    constructCells: function (idAabb)
    {
    ...
    intersectRay: function (vNear, rayInv)
    {
        if (this.aabbLoose_.intersectRay(vNear, rayInv))
        {
            if (this.child_[0])
            {
                ...
                for (var i = 0; i < 8; ++i)
                {
                    var indices = child[i].intersectRay(vNear, rayInv);
    ...
    intersectPlane: function (origin, normal)
    {
        if (this.aabbLoose_.intersectPlane(origin, normal))
        {
            if (this.child_[0])
            {
                ...
                for (var i = 0; i < 8; ++i)
                {
                    var indices = child[i].intersectPlane(origin, normal);
