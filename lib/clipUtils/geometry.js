...
Geometry.computeGeometryAabb = function (aabb, vertexList)
{
    ...
    max = -Infinity;
    var xyz = 0;
    for (var i, n = vertexList.length ; i < n ; i++) {
...
Geometry.pointOnLine = function (v, vNear, vFar)
{
    ...
    var temp = [0, 0, 0];
    var dot = vec3.dot(ab, vec3.sub(temp, v, vNear));
...
{
    var tmp = [0, 0, 0];
    var distToPlane = vec3.dot(vec3.sub(tmp, v1, origin), normal);
    var dotNormals = vec3.dot(normal, vec3.normalize(tmp, vec3.sub(tmp, v2, v1)));
    var t = -distToPlane / dotNormals;
